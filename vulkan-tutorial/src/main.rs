...
impl App
{
    ...
    unsafe fn render(&mut self, window: &Window) -> Result<()> {
        ...
        if !self.data.images_in_flight[image_idx as usize].is_null() {
            self.device.wait_for_fences(
                &[self.data.images_in_flight[image_idx as usize]],
                true,
                u64::MAX,
            )?;
        }
        ...
        self.data.images_in_flight[image_idx as usize] =
            self.data.images_in_flight[self.frame];

        let wait_semaphores = &[self.data.image_available_semaphores[self.frame]];
        let wait_stages = &[vk::PipelineStageFlags::COLOR_ATTACHMENT_OUTPUT];
        let command_buffers = &[self.data.command_buffers[image_idx as usize]];
    ...
    unsafe fn destroy(&mut self)
    {
        ...
        self.data.swapchain_images_views
            .iter()
...
/// Create a Vulkan instance with a Window and Entry point passed as an argument.
/// ### Args
/// - window (Window): a winit Window.
/// - entry (Entry): a vulkan entry point.
...
) -> Result<Instance>
{
    ...
    // Add a Messenger Callback when the Validation Layers are enabled
    if VALIDATION_ENABLED {
        extensions.push(vk::EXT_DEBUG_UTILS_EXTENSION.name.as_ptr());
    }

    // check macOS compatibility if we are on macOS
    ...
    // create with the global extension a Vulkan instance using the entry point.
...
/// Determine whether a physical device meets our needs.
...
) -> Result<()>
{
    ...
    // TODO: change this later to an EXCLUSIVE MOD only
...
{
    let vert = include_bytes!("../shaders/hello_triangle/vert.spv");
    let frag = include_bytes!("../shaders/hello_triangle/frag.spv");
